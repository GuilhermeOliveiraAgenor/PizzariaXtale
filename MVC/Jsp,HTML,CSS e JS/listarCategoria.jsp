...
<body>
 <div>
    ...
    <h1 class="h1">Listar categoria</h1>
    <table id="customers">
    <thead>
        <tr>
            <th class = "th">Id Categoria</th>
            <th class = "th">Nome</th>
        </tr>
        </thead>
        <tbody>
			...
			%>
			<tr>
				<td class = "td"><%=listar.get(i).getIdCategoria()%></td>
				<td class = "td"><%=listar.get(i).getNome()%></td>
				<td><a	class = "botao" href="selecionarcategoriAlterar?idCategoria=<%=listar.get(i).getIdCategoria()%>">
						ALTERAR</a></td>
				<td><a class = "botao" href="javascript: confirmar(<%=listar.get(i).getIdCategoria()%>)">
						EXCLUIR</a></td>
			</tr>
			<%
			}
	...
	<script>
function confirmar(idCategoria){
...
if(resposta === true){
	...
	//alert(Cpf)
	window.location.href = "excluirCategoria?idCategoria=" + idCategoria;
}
